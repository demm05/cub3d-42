...
static inline void	text_loop(t_engine *eng, FT_Bitmap *bitmap,
	...
	while (++bmp.y < (int)bitmap->rows)
	{
		...
		while (++bmp.x < (int)bitmap->width)
		{
			...
			if (screen.x > eng->window.width)
				break ;
			blend_normal_a(eng, screen, color,
				bitmap->buffer[bmp.y * bitmap->width + bmp.x]);
		}
	}
}
	...
	t_point baseline, t_ui color)
{
	...
	text_loop(eng, bitmap, offset, color);
}
